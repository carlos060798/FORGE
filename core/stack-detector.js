/**
 * stack-detector.js — Detección determinista del stack del proyecto
 */
 ...
 * @property {string} test_cmd
 * @property {string|null} lint_cmd
 * @property {string|null} build_cmd
 * @property {string} install_cmd
 * @property {'alta'|'media'|'baja'} confianza
 * @property {string[]} señales
 */

/** @param {string} [cwd] @returns {StackInfo} */
export function detectStack(cwd = process.cwd()) {
  ...
  const contains = (content, ...terms) => terms.some(t => content.toLowerCase().includes(t.toLowerCase()));
  ...
  if (exists('package.json')) {
    ...
    const lenguaje = isTS ? 'typescript' : 'javascript';
    ...
    else if (pkgJson['scripts']?.['test']) { señales.push('scripts.test definido en package.json'); }
    ...
    if (pkgJson['scripts']?.['build']) { build_cmd = 'npm run build'; }
    ...
    const base_datos = _detectDb(deps, señales);

    return { lenguaje, runtime: 'Node.js', framework, base_datos, test_cmd, lint_cmd, build_cmd, install_cmd, confianza: 'alta', señales };
  }
  ...
  if (exists('pyproject.toml') || exists('setup.py') || exists('requirements.txt')) {
    ...
    return {
      lenguaje: 'python', runtime: 'CPython', framework,
      base_datos: _detectPythonDb(combined, señales), test_cmd,
  ...
  if (exists('go.mod')) {
    ...
    return { lenguaje: 'go', runtime: 'Go', framework, base_datos: null, test_cmd: 'go test ./...', lint_cmd: 'golangci-lint run', build_cmd: 'go build ./...', install_cmd: 'go mod download', confianza: 'alta', señales };
  }
  ...
  if (exists('Cargo.toml')) {
    ...
    return { lenguaje: 'rust', runtime: 'Rust', framework, base_datos: null, test_cmd: 'cargo test', lint_cmd: 'cargo clippy', build_cmd: 'cargo build --release', install_cmd: 'cargo fetch', confianza: 'alta', señales };
  }
  ...
  if (exists('pom.xml') || exists('build.gradle') || exists('build.gradle.kts')) {
    ...
    return { lenguaje: 'java', runtime: 'JVM', framework, base_datos: null, test_cmd: isMaven ? 'mvn test -q' : './gradlew test', lint_cmd: null, build_cmd: isMaven ? 'mvn package -q' : './gradlew build', install_cmd: isMaven ? 'mvn dependency:resolve -q' : './gradlew dependencies', confianza: 'alta', señales };
  }
  ...
  if (exists('Gemfile') || hasGemspec) {
    ...
    return { lenguaje: 'ruby', runtime: 'Ruby', framework, base_datos: null, test_cmd: exists('spec') ? 'bundle exec rspec' : 'bundle exec rake test', lint_cmd: 'bundle exec rubocop', build_cmd: null, install_cmd: 'bundle install', confianza: 'alta', señales };
  }
  ...
  if (exists('composer.json')) {
    ...
    return { lenguaje: 'php', runtime: 'PHP', framework, base_datos: null, test_cmd: exists('vendor/bin/phpunit') ? './vendor/bin/phpunit' : 'php artisan test', lint_cmd: exists('vendor/bin/phpstan') ? './vendor/bin/phpstan analyse' : null, build_cmd: null, install_cmd: 'composer install', confianza: 'alta', señales };
  }

  return { lenguaje: 'unknown', runtime: 'unknown', framework: null, base_datos: null, test_cmd: 'echo "No se detectó framework de tests"', lint_cmd: null, build_cmd: null, install_cmd: 'echo "No se detectó gestor de dependencias"', confianza: 'baja', señales: ['Ningún manifest reconocido encontrado'] };
}

function _detectDb(deps, señales) {
  const hasDep = (...names) => names.some(n => n in deps);
...
function _detectPythonDb(content, señales) {
